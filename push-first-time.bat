...
rem 版本号：V1.1
rem 时间：20170324
rem 功能：自动把本地项目文件夹上传到云端GitHub和码云，仅适用于第一次提交（只用一次，提交到云端空文件夹）
...
echo -------------------create remote git-osc----------------------
git remote rm git-osc
rem 这句代码的git@git.oschina.net:xueshanlinghu/learn-git-and-github.git改成你自己的远端地址哦~
git remote add git-osc git@git.oschina.net:xueshanlinghu/learn-git-and-github.git
echo.
echo --------------now we push the data to git-osc-----------------
git push -u git-osc master
git remote rm git-osc
